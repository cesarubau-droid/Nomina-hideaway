# ============================================================
# CALCULADOR SEGURIDAD — v4.0
# Reglas:
#   Con Acuerdo (15, 16, 22, 23h): 6h ord + 1h extra noc fija
#   Sin Acuerdo (6h): 8h ord + 1h extra + regla 20/45
#   Sin Acuerdo (8h): 8h ord + regla 20/45
...
def calcular(fecha: str, punches_raw: list, es_nocturno: bool = False,
    ...
    """
    Calcula horas para un empleado de Seguridad.
    ...
    if es_nocturno and exit_str:
        ...
        if turno_h in SEG_ACUERDO_STARTS:
            return _con_acuerdo(re_m, nota_fer)
        else:
    ...
    if turno_h in SEG_ACUERDO_STARTS:
        nota = nota_fer if nota_fer else 'Acuerdo Seg: 6h ord + 1h extra noc'
        if is_late:
            nota = f'Tardío + {nota}'
        return _con_acuerdo(re_m, nota)
    else:
        entry_count = re_m + LATE_PENALTY if is_late else re_m
        if exit_m <= entry_count:
            exit_m += 24 * 60
        return _sin_acuerdo(entry_count, exit_m, turno_h, nota_fer, es_confianza, is_late, re_m, entry_str)


def _con_acuerdo(re_m: int, nota: str) -> dict:
    """6h ordinarias + 1h extra nocturna fija."""
    ord_end = re_m + 6 * 60
    d_o, mx_o, n_o = split_hours(re_m, ord_end)
    if not nota:
        nota = 'Acuerdo Seg: 6h ord + 1h extra noc'
    return {
...
def _sin_acuerdo(entry_count: int, exit_m: int, turno_h: int,
    ...
    Turno 6h: también tiene 1h extra fija además de la regla 20/45.
    """
    ...
    xd = xm = xn = 0
    # Turno 6h tiene 1h extra fija
    if turno_h == 6:
        xn = 1.0

    # Regla 20/45 (solo si no es confianza)
